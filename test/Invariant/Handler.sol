...
contract Handler is CommonBase, StdCheats, StdUtils {
    ...
    function burn(uint256 seed, uint256 amount) public useActor(seed) countCall("burn") {
    ...
    function disable(uint256 seed, bool flag) public useActor(seed) countCall("disableRebase") {
        if (currentActor != address(0) && flag != ustb.optedOut(currentActor)) {
            vm.startPrank(currentActor);
        ...
        countCall("transfer")
    {
        ...
        if (currentActor != address(0)) {
            ...
            ghost_zeroAddressTransfer++;
        }
    }

    function sendFrom(uint256 actorSeed, uint256 toSeed, uint256 amount)
        public
        useActor(actorSeed)
        countCall("sendFrom")
    {
        ...
        if (currentActor != address(0)) {
            ...
            (nativeFee,) = ustb.estimateSendFee(uint16(block.chainid), abi.encodePacked(to), amount, false, "");

            uint256 contractBalBeforeBridge = ustb.balanceOf(address(this));

            ustb.sendFrom{value: (nativeFee * 105) / 100}(
                currentActor, uint16(block.chainid), abi.encodePacked(to), amount, payable(currentActor), address(0), ""
            );

            uint256 contractBalAfterBridge = ustb.balanceOf(address(this));

            ghost_bridgedTokensTo += contractBalAfterBridge - contractBalBeforeBridge;

            vm.startPrank(to);

            (nativeFee,) = ustb.estimateSendFee(
                uint16(block.chainid), abi.encodePacked(currentActor), ustb2.balanceOf(to), false, ""
            );
            ...
            ghost_bridgedTokensFrom += contractBalBeforeBridge0 - contractBalAfterBridge0;
        } else {
            ghost_zeroAddressSendFrom++;
        }
    }
    ...
    function __mint(address addr, uint256 amount) internal {
        (bool success,) = address(usdm).call(abi.encodeWithSignature("mintTokens(address,uint256)", addr, amount));
        assert(success);
    }
}
